...
void SSP0Init( void )
{
 ...
 // LPC_PINCON->PINSEL1 |= ((0x2<<0)|(0x2<<2)|(0x2<<4));
	// LPC_PINCON->PINMODE1 |= ((0x3<<0)|(0x3<<2)|(0x3<<4)); //PULLDOWN MODE
...
/*****************************************************************************
** Function name:		SSPTrans
**
** Descriptions:		Send and receive a block of data to the SSP port, the 
**						first parameter is the buffer pointer, the 2nd 
**						parameter is the block length.
**
** parameters:			Send buffer pointer, Receive buffer pointer, and the block length
** Returned value:		None
** 
*****************************************************************************/
void SSPTrans( uint32_t portnum, uint8_t *send_buf,uint8_t *receive_buf, uint32_t Length )
{
	 uint32_t i;
  ...
  for ( i = 0; i < Length; i++ )
  {
    ...
    {
		 /* Move on only if NOT busy and TX FIFO not full. */
	  while ( ((LPC_SSP0->SR & (SSPSR_TNF|SSPSR_BSY)) != SSPSR_TNF ) & !(LPC_SSP0->SR & SSPSR_RNE));
	  LPC_SSP0->DR = *send_buf;
	  send_buf++;
	  while ( (LPC_SSP0->SR & (SSPSR_BSY|SSPSR_RNE)) != SSPSR_RNE );
	  *receive_buf++ = LPC_SSP0->DR;
	}
	 if ( portnum == 1 )
    {
		 /* Move on only if NOT busy and TX FIFO not full. */
	  while ( ((LPC_SSP1->SR & (SSPSR_TNF|SSPSR_BSY)) != SSPSR_TNF ) & !(LPC_SSP1->SR & SSPSR_RNE));
	  LPC_SSP1->DR = *send_buf;
	  send_buf++;
	  while ( (LPC_SSP1->SR & (SSPSR_BSY|SSPSR_RNE)) != SSPSR_RNE );
	  *receive_buf++ = LPC_SSP1->DR;
	}
